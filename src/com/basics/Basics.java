...
public class Basics {
    ...
    public List<String> generateParenthesis(int positionToBeFilled, int leftBraceCount, int rightBraceCount, int len) {
        List<String> resultSet = new ArrayList<>();
        StringBuilder ans = new StringBuilder();
        if (positionToBeFilled < len && (leftBraceCount <= rightBraceCount)) {
            ans.append('(');
            generateParenthesis(positionToBeFilled + 1, ++leftBraceCount, rightBraceCount, len);
