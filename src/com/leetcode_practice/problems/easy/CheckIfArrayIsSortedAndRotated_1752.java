...
public class CheckIfArrayIsSortedAndRotated_1752 {
    ...
    public boolean check(int[] nums) {
        ...
        if (nums[n - 1] > nums[0])
            count++;// There should be max one incident where the higher index element is smaller
                    // than
        // lower index element in the circular chain.
        return count <= 1 ? true : false;
    }

    // We can treat the array as circular
    public boolean check_betters(int[] nums) {
        int count = 0;
        int n = nums.length;
        for (int i = 1; i < n; i++) {
